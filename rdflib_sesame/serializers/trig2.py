...
__all__ = ['TrigSerializer2']


class TrigSerializer2(TurtleSerializer):
    ...
    def __init__(self, store):
        ...
        if store.context_aware:
            print(list(store.contexts()), store.default_context.identifier)
        ...
        super(TrigSerializer2, self).__init__(store)

    def preprocess(self):
        for context in self.contexts:
            self.store = context
            self.getQName(context.identifier, gen_prefix=False)
    ...
    def reset(self):
        super(TrigSerializer2, self).reset()
        self._contexts = {}
    ...
    def serialize(self, stream, base=None, encoding=None,
        ...
        for store, (ordered_subjects, subjects, ref) in self._contexts.items():
            ...
            print(self.default_context,store.identifier, self.default_context )
